 ...
 *   Tag:   hesap-merkezi-2.2.10   (TAG_PREFIX + sürüm)
 *   Asset: hesap-merkezi.zip
 */
class MHM_Updater {

    const CENTRAL_URL = 'https://api-production-76ce.up.railway.app';
    const PLUGIN_SLUG_ID = 'hesap-merkezi'; // merkez API'de ?slug= parametresi
    const GITHUB_REPO = 'adminmagazify/wp-plugins';
    const CACHE_KEY   = 'mhm_update_check';
    ...
    protected static function get_latest_release() {
        ...
        $best = [
            ...
            'name'      => isset($data['name']) ? $data['name'] : 'Magazify Hesap Merkezi',
        ];
    ...
    public static function check_for_update($transient) {
        ...
        $current = isset($transient->checked[self::$plugin_file])
            ...
            : (defined('MHM_VERSION') ? MHM_VERSION : '0');
    ...
    public static function plugin_info($result, $action, $args) {
        ...
        return (object) [
            'name'          => !empty($data['Name']) ? $data['Name'] : 'Magazify Hesap Merkezi',
            'slug'          => self::$plugin_slug,
            'version'       => $version,
            ...
            'sections'      => [
                'description' => !empty($data['Description']) ? $data['Description'] : 'WooCommerce Hesabım menüsünü Magazify için düzenler.',
                'changelog'   => $changelog,
            ],
        ];
    }
